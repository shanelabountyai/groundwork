...
**Pick up:** DG-12 needs artboards first (properties list/detail, agreements new/detail, users, crews, service types, invoice create, property new), then build. DG-11 is done (CrewShift + DispatchRouteTools built, 2026-09-26).
...
**Exec-brief screenshots: published 2026-09-25** (version 2 of https://claude.ai/artifact/HX28FGarRjEQzkLiMyrReU, `dispatch-board.png` + `crew-day.png` as published files). Closure deliverables are complete. The Artifact tool works again since `ANTHROPIC_API_KEY` was taken out of `~/.zshrc`.
...
| DG-04 | **Done 2026-09-25** (`src/forms.tsx`; service types, crews, properties, agreements, jobs, users; `npm test` 150/150, e2e 27/27). Checkbox `notifyOnEnRoute` is not refilled on a bounce. Inline per-field form errors (`.err` under the field). Forms still redirect with one flash message. | every `actions.ts` with a form | A bad price marks that field and keeps the rest of the input; no client JS. |
...
| CG-09 | **Done 2026-09-25** (one month per view, `?month=` prev/next GET links, defaults to the first month with a weekday; `npm test` 150/150, portal e2e 6/6). The reschedule calendar renders every month in the 60-day window (about three cards) on a phone. | Consider showing one month with prev/next links (GET). |
