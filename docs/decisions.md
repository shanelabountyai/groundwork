...
## SEC-04: per-IP link limit and indexed phone lookup (2026-09-25)
...
## Gap closures (2026-09-26): retry after decline, drag reorder, outbox cron

- **A declined reschedule is retryable.** The visit is already skipped, so a retry is just another `requestReschedule`: when the visit's latest request is `declined` (and the property owns it) the skip step is a no-op, then the normal book-or-queue path runs. A full day queues a *new* `RescheduleRequest`; the old declined row stays as history. The portal shows only a visit's latest request, and hides a declined one once a make-up is booked in its slot. The make-up slot's unique index still allows exactly one booking. A pending latest request refuses a retry (the skip throws `IllegalTransition`).
- **Drag reorder is HTML5 drag-and-drop on the server-rendered list** (`app/dispatch/drag-list.tsx`), posting the whole order to `dropStops` → `reorderRoute`. ↑/↓ stay as the keyboard and touch path, since HTML5 DnD does not fire from touch. A stale list (a stop arrived mid-drag) reloads instead of erroring.
- **The outbox drains from Vercel Cron** (`vercel.json`, every 10 min → `/api/cron/outbox`), because a script cannot be a cron target on Vercel. The route needs `Authorization: Bearer $CRON_SECRET` and refuses when the secret is unset. Vercel's cron on the Hobby plan runs at most daily; `*/10` needs Pro. `npm run outbox:drain` remains for local use. Set `CRON_SECRET` in the Vercel project before deploying.

## Gap closures (2026-09-26, 2): decline notice, generation capacity, report CSV

- **Declining a reschedule queues a customer notice**, written in the same transaction as the decision (`declineReschedule`), so there is no decline without a notice and no notice for a decline that rolled back. The body carries the dispatcher's note and points at the portal.
- **Capacity on horizon generation and agreement edits is a warning, not a gate.** A pattern's visits must exist (rule 5), so refusing would silently drop revenue; `generateVisits`, `createAgreement` and `editAgreement` return `overloaded` (crew-days left over capacity by *this* run: newly placed visits, plus every attached future visit on a crew change) via `overloadedDays`. The CLI prints them, and agreement save messages name the first three days. Override rows are not written: nobody chose to overload. Not covered: overloads that already existed and were not touched by the run.
- **The weekly report exports at `/dispatch/report/csv?week=`**, the page's table as CSV plus an invoiced/collected row (dollars from integer cents). Weekly only; the quarter view still has no export.
