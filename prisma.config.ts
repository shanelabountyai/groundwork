...
// catches. Reaching one on purpose has to be typed: GROUNDWORK_ALLOW_CLOUD_DB=1.
if (!process.env.GROUNDWORK_ALLOW_CLOUD_DB && /neon\.tech|rds\.amazonaws|supabase\.co/.test(url)) {
  throw new Error('Groundwork runs on local Postgres. Refusing a cloud database.');
}
