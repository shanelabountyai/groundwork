...
import { defaultProvider, type Provider } from './provider';

/** Drains outbox rows (sentAt IS NULL) oldest first. Stamps sentAt only after a successful send. */
export async function drainOutbox(provider: Provider = defaultProvider, clock: Clock = systemClock, limit = 50) {
